...
def test_extract_addresses_from_dump_unknown_file():
    ...
    with pytest.raises(EnvironmentError):
        parser.extract_cycles_from_rocket_log(
...
def test_extract_addresses_from_exec_log_unknown_file():
    parser = LogParser()
    with pytest.raises(EnvironmentError):
        parser.extract_addresses_from_dump(dump_file="non_existing_file")
...
def test_extract_addresses_from_dump(mocker):
    ...
    (start_address, end_address) = parser.extract_addresses_from_dump("mocked_data")
...
def test_extract_cycles_from_exec_log(mocker):
    mocker.patch("builtins.open", mocker.mock_open(read_data=exec_log_data))
    parser = LogParser()
    # Triggering the open will run into the mocked file!
    start_cycle, end_cycle = parser.extract_cycles_from_rocket_log(
